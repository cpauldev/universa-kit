...
const overlayFontsDir = path.resolve(rootDir, "src/overlay/fonts");
...
async function inlineOverlayCss() {
  const [rawCss, overlayBundle, fontFiles] = await Promise.all([
    ...
    Promise.all(
      [
        ...
        fileName,
        await fs.readFile(path.join(overlayFontsDir, fileName)),
      ]),
    ),
  ]);
  const fontDataUrls = new Map(
    fontFiles.map(([fileName, buffer]) => [
      fileName,
  ...
  const css = rawCss.replace(
    /url\((?:["'])?\.\/fonts\/([^"')/]+)(?:["'])?\)/g,
    (match, fileName) => fontDataUrls.get(fileName) ?? match,
  );

  const marker = /(["'`])__UNIVERSAL_OVERLAY_CSS_INLINE__\1/;
  const placeholder =
  ...
  if (marker.test(patched)) {
    ...
    throw new Error(
      "Failed to inline overlay CSS: marker __UNIVERSAL_OVERLAY_CSS_INLINE__ was not found in dist/overlay/overlay.js",
    );
  }
